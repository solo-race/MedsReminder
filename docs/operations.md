...
| Chinese IME swallows `input text` | Typed text does not reach Compose fields (`uiautomator` shows `text=''`) | Disable the keyboard for the run: `adb shell ime disable com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME`, type, then restore with `ime enable` + `ime set` on the same id (on PLB110 the Google Latin IME is the only installed one and still swallows injected keys while active) |
...
| Warm reminder delivery appears to do nothing | `am start -n <pkg>/.MainActivity --el …` against a running app prints "intent has been delivered to currently running top-most instance" and AMS logs `result code=3`, but the app's `onNewIntent` never runs on PLB110/ColorOS unless the activity's launch mode matches the incoming intent | Declare the entry activity `android:launchMode="singleTop"` (2026-09-14 fix); to test the app-side path independently of that OEM decision, add `--activity-single-top` to the adb command, which always delivers through `onNewIntent` |
| App log output missing | `Log.i`/`Log.w` lines from the app never appear in `logcat` for this package on PLB110 | Probe through an app-private file instead: append to `files/<probe>.txt`, read with `adb exec-out run-as <pkg> cat files/<probe>.txt`, and delete the probe (and its file) before producing the final artifact |
| `install -r` leaves the old process running | A freshly installed fix appears to have no effect because the app keeps executing the pre-install code | Compare the process start time (`ps -A -o PID,ETIME,ARGS \| grep <pkg>` or `stat -c %y /proc/<pid>`) with the install time; kill a stale process with `adb shell run-as <pkg> kill -9 <pid>`, then relaunch before testing |
| Notification shade is not automatable | `uiautomator dump` returns zero nodes for the shade, and the OEM auto-groups reminders (`g:Aggregate_AlertingSection`) | Locate cards from a screenshot (vision) and tap by coordinate: first tap on the group card expands it, then the child card fires the app's content `PendingIntent`; confirm the fired intent with `dumpsys notification --noredact` (`contentIntent`, `when`, `actions`) |
